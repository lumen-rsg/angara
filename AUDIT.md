...
| - [x] **LIB-6** | ✅ Fixed | Medium | ~~No HTTP server (websocket server is a no-op-close PoC)~~ — new `http` module: raw-socket HTTP/1.1 server with `server()`/`poll()`/`respond()`, dual-stack IPv6/IPv4, header parsing, keep-alive support. | `modules/net/http_server.c` |
...
| - [x] **LIB-8** | ✅ Fixed | Medium | ~~DB drivers: sqlite only~~ — now also `redis` (hiredis), `postgres` (libpq), `mysql` (libmariadb); ~~no connection pooling~~ → `sqlite.pool()` with acquire/release; ~~no explicit transaction API~~ → `begin()`/`commit()`/`rollback()` on SqliteDb. | `modules/data/sqlite.c` |
...
| - [x] **LIB-11** | ✅ Fixed | Low | ~~No file watching~~ → new `watch` module (Linux inotify + BSD/macOS kqueue); ~~no zstd/bzip2/xz~~ → new `compress` module (zstd + bzip2 + xz/lzma); ~~date/time is UTC-only~~ → added `format_local`, `date_parts_local`, `timezone_offset`, `timezone_name`. | — |
...
- **~~kqueue (macOS / BSD)~~** ✅ Fixed — The `watch` module now has a
  `#ifdef __linux__` / `#else` split with a full kqueue (`EVFILT_VNODE`)
  backend. Event mask constants are portable across platforms.
  **Note: the kqueue path is currently untested** — no macOS/BSD CI
  is available.
