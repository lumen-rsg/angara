...
| - [ ] **BUG-9** | 🟡 Source | Medium | **`finalize` only handles `OBJ_RECORD`**, not the record-layout aliases (CLASS/DATA_INSTANCE/ENUM_INSTANCE) → their `entries` arrays and `strdup`'d keys leak on collection. _(Scope corrected during Phase 1 verification: `finalize` actually handles STRING/LIST/RECORD/NATIVE — 4 tags, not only RECORD — but the gap is broader: CLASS/INSTANCE/DATA_INSTANCE/ENUM_INSTANCE/EXCEPTION/CLOSURE are all un-finalized. Deferred to Phase 2.)_ | `rt/MarkSweepGC.cpp:678-682` |
...
| - [ ] **BUG-13** | 🟡 Source | Medium | **GC root frame capped at 256 slots per function** — functions with >256 tracked locals silently drop roots and get collected while still reachable. No overflow diagnostic. | `angc/includes/LLVMBackend.h`; `TopLevel.cpp:168,286` |
...
| - [ ] **GC-6** | 🟡 Source | Medium | **Recursive marking with no explicit mark stack** → deep object graphs (~10k+ depth) exhaust the native stack. | `MarkSweepGC.cpp:365-411` |
| - [ ] **GC-7** | 🟡 Source | Medium | **`count` reset to 0 on collect, not to survivors** → collection frequency wrong for stable working sets (collects after N *fresh* allocations, not N live objects). | `MarkSweepGC.cpp:985`; `ChaperoneGC.cpp:1397` |
