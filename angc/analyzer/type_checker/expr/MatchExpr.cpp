...
namespace angara {
    ...
    std::any TypeChecker::visit(const MatchExpr& expr) {
        ...
        if (condition_type->kind != TypeKind::ENUM) {
            error(expr.keyword, "Match expressions can only be used with enum types, but got '" + condition_type->toString() + "'.");
            pushAndSave(&expr, m_type_error);
            return {};
        }
        ...
        for (const auto& case_item : expr.cases) {
            ...
            if (pattern_type->kind == TypeKind::FUNCTION) {
                ...
                if (func_type->return_type->toString() != enum_type->name) {
                    error(expr.keyword, "Variant '" + variant_name + "' does not belong to enum '" + enum_type->name + "'.");
                } else {
                ...
                if (case_item.variable) {
                    if (func_type->param_types.empty()) {
                        error(*case_item.variable, "Variant '" + variant_name + "' has no payload to bind.");
                    } else {
                    ...
                    if (!func_type->param_types.empty()) {
                        error(expr.keyword, "Match case for variant '" + variant_name + "' must bind its payload to a variable, e.g., 'case " + variant_name + "(x): ...'.");
                    }
                }
            }
        ...
        if (!has_wildcard && covered_variants.size() != enum_type->variants.size()) {
            error(expr.keyword, "Match expression is not exhaustive. Handle all variants of '" + enum_type->name + "' or add a wildcard case '_'.");
        }
