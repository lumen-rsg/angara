...
std::shared_ptr<Stmt> Parser::classDeclaration() {
        ...
        if (match({TokenType::INHERITS})) {
            Token super_name = consume(TokenType::IDENTIFIER, "Expected superclass name after 'inherits'.");
            superclass = std::make_shared<VarExpr>(super_name);
        }
        std::vector<std::shared_ptr<VarExpr>> traits;
        if (match({TokenType::USES})) {
            do {
                Token trait_name = consume(TokenType::IDENTIFIER, "Expected trait name after 'uses'.");
        ...
        if (match({TokenType::SIGNS})) {
            do {
                Token contract_name = consume(TokenType::IDENTIFIER, "Expected contract name after 'signs'.");
        ...
        while (!check(TokenType::RIGHT_BRACE) && !isAtEnd()) {
            if (match({TokenType::PUBLIC})) {
                consume(TokenType::COLON, "Expected ':' after 'public' access specifier.");
                current_access = AccessLevel::PUBLIC;
                continue;
            }
            if (match({TokenType::PRIVATE})) {
                consume(TokenType::COLON, "Expected ':' after 'private' access specifier.");
            ...
            if (match({TokenType::LET}) || match({TokenType::CONST})) {
                ...
                throw error(peek(), "Expected a member declaration ('let', 'const', 'func') or access specifier ('public:', 'private:') in class body.");
            }
        }
