...
namespace angara {
    ...
    std::shared_ptr<Expr> Parser::matchExpression() {
        ...
        consume(TokenType::RIGHT_PAREN, "Expected ')' after 'match' condition.");
        consume(TokenType::LEFT_BRACE, "Expected '{' to begin 'match' body.");

        std::vector<MatchCase> cases;

        while (!check(TokenType::RIGHT_BRACE) && !isAtEnd()) {
            consume(TokenType::CASE, "Expected 'case' to begin a match arm.");
            std::shared_ptr<Expr> pattern = primary();
            while (match({TokenType::DOT})) {
                Token op = previous();
                Token name = consume(TokenType::IDENTIFIER, "Expected property name after '.' in match pattern.");
                pattern = std::make_shared<GetExpr>(std::move(pattern), op, std::move(name));
            }
            std::optional<Token> variable = std::nullopt;
            if (match({TokenType::LEFT_PAREN})) {
                variable = consume(TokenType::IDENTIFIER, "Expected a variable name to bind the enum variant's payload.");
                consume(TokenType::RIGHT_PAREN, "Expected ')' after pattern variable.");
            }
            ...
            if (match({TokenType::LEFT_BRACE})) {
                body = expression();
                consume(TokenType::RIGHT_BRACE, "Expected '}' after match case body.");
            } else {
                body = expression();
            }
            ...
            if (!check(TokenType::RIGHT_BRACE)) {
                consume(TokenType::COMMA, "Expected ',' between match cases, or '}' to close 'match'.");
            }
        }
