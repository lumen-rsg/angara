...
AngaraObject Angara_Channel_consume(int arg_count, AngaraObject* args) {
    ...
    // --- REVISED TIMEOUT LOGIC ---
    struct timeval* timeout = NULL; // Default to blocking indefinitely.
    struct timeval tv;
    if (arg_count == 3 && IS_I64(args[2])) {
        int64_t ms = AS_I64(args[2]);
        if (ms >= 0) { // Only set a timeout for non-negative values.
            tv.tv_sec = ms / 1000;
    ...
    if (res.reply_type != AMQP_RESPONSE_NORMAL) {
        if (res.library_error == AMQP_STATUS_TIMEOUT) {
            return angara_create_nil(); // Return nil on timeout, as before.
        }
        check_amqp_reply(res, "Consuming message");
        amqp_destroy_envelope(&envelope); // Must destroy envelope on error path too.
        return angara_create_nil();
    }
    ...
    // 3. Correlation ID (optional property)
...
AngaraObject Angara_Channel_bind_queue(int arg_count, AngaraObject* args) {
    ...
    // Get the channel and connection data
    ChannelData* ch_data = get_channel_data(args[0]);
    ConnectionData* conn_data = get_conn_data_from_channel(ch_data);

    // Unbox the arguments from AngaraObjects to C strings
    const char* queue_name = AS_CSTRING(args[1]);
    const char* exchange_name = AS_CSTRING(args[2]);
    const char* routing_key = AS_CSTRING(args[3]);

    // Call the underlying librabbitmq-c function
...
AngaraObject Angara_Channel_exchange_declare(int arg_count, AngaraObject* args) {
    ...
    amqp_exchange_declare(conn_data->conn, ch_data->id, amqp_cstring_bytes(exchange_name),
                          amqp_cstring_bytes(exchange_type), passive, durable, auto_delete, 0, amqp_empty_table);
...
AngaraObject Angara_Channel_prefetch(int arg_count, AngaraObject* args) {
    ...
    // The `prefetch_count` is a uint16_t.
    uint16_t prefetch_count = (uint16_t)AS_I64(args[1]);

    // amqp_basic_qos is the function for setting prefetch.
    // prefetch_size = 0 (no limit)
    // global = false (apply this prefetch to this channel only)
    amqp_basic_qos(conn_data->conn, ch_data->id, 0, prefetch_count, 0);
    check_amqp_reply(amqp_get_rpc_reply(conn_data->conn), "Setting prefetch (QOS)");
