//
// color.c — Angara terminal color module
//
// Provides ANSI color/formatting codes for terminal output.
// Functions return strings that can be concatenated with text.
// Also provides direct print helpers.
//
...
// Helper: return ANSI escape string
...
// --- color.fg(code) -> string ---
// 256-color foreground: code 0-255
...
// --- color.bg(code) -> string ---
// 256-color background: code 0-255
...
// --- color.rgb_fg(r, g, b) -> string ---
// True-color foreground (24-bit)
...
// --- color.rgb_bg(r, g, b) -> string ---
// True-color background (24-bit)
...
// --- color.style(text) -> string ---
// Applies reset + text + reset for safe coloring
AngaraObject Angara_color_style(int arg_count, AngaraObject* args) {
    ...
    // Build: codes... + text + reset
    size_t text_len = ang_api->str_len(args[0]);
    size_t total = text_len + 4 + 4; // reset + text + reset
