//
// csv.c — Angara CSV module
//
// Provides CSV parsing and generation with support for:
//   - Custom delimiters (default: comma)
//   - Quoted fields (double-quote escaping)
//   - Newline handling within quoted fields
//   - Header-based record parsing
//
...
// Parse a single field. Handles quoting and escaping.
...
static char* parse_field(CsvParser* p) {
    ...
    if (parser_peek(p) == '"') {
        // Quoted field
        parser_next(p); // consume opening quote
        while (p->pos < p->len) {
...
// Parse one row. Returns a list of strings.
static AngaraObject parse_row(CsvParser* p) {
    ...
    while (1) {
        ...
        char c = parser_peek(p);
        if (c == p->delimiter) {
            parser_next(p); // consume delimiter
            // If at end of line after delimiter, add empty trailing field
...
// ---------------------------------------------------------------------------
// csv.parse(text, delimiter?) -> list<list<string>>
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// csv.parse_record(text, delimiter?) -> list<record>
// First row is treated as headers, subsequent rows become records.
// ---------------------------------------------------------------------------
...
// rows is a list<list<string>>. Properly quotes fields containing delimiters,
// quotes, or newlines.
// ---------------------------------------------------------------------------
...
AngaraObject Angara_csv_stringify(int arg_count, AngaraObject* args) {
    ...
    for (size_t r = 0; r < num_rows; r++) {
        ...
        for (size_t f = 0; f < num_fields; f++) {
            ...
            if (needs_quote) EMIT('"');
            for (size_t i = 0; i < val_len; i++) {
                if (val[i] == '"') EMIT('"'); // Escape double quote
                EMIT(val[i]);
            }
            if (needs_quote) EMIT('"');
...
// ---------------------------------------------------------------------------
// csv.read_file(path, delimiter?) -> list<list<string>>
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// csv.write_file(path, rows, delimiter?) -> nil
// ---------------------------------------------------------------------------
...
AngaraObject Angara_csv_write_file(int arg_count, AngaraObject* args) {
    ...
    // Reuse stringify to build CSV text
