//
// jwt.c — Angara JSON Web Token (JWT) module
//
// Provides JWT creation, verification, and decoding.
// Supports HS256, HS384, HS512 algorithms.
// Uses json_bridge for JSON and manual HMAC implementation.
//
...
// SHA-256 / SHA-384 / SHA-512 context structures
// We use CommonCrypto on macOS and OpenSSL on Linux
// But to avoid external deps, we implement HMAC using the hash module's approach

// Portable HMAC using a simple hash function
// We'll use a mini SHA-256/512 implementation
...
// ---------------------------------------------------------------------------
// JSON ↔ Angara conversion (same as rpc.c)
// ---------------------------------------------------------------------------
...
// --- jwt.create(payload, secret, algorithm?) -> string ---
...
// --- jwt.verify(token, secret) -> record? ---
// Returns the payload if valid, nil if invalid.
...
// --- jwt.decode(token) -> record ---
// Decodes the payload without verification. Returns {header, payload}.
