//
// net.c — Angara raw networking module
//
// Provides TCP/UDP sockets, DNS resolution, and IP address utilities.
// No external dependencies — pure POSIX sockets + DNS.
//
...
// ---------------------------------------------------------------------------
// TCP Connection (native instance)
// ---------------------------------------------------------------------------
...
// --- net.tcp_connect(host, port, timeout_ms?) -> Connection ---
...
// --- TcpConn.recv_line(timeout_ms?) -> string? ---
// Reads until '\n' or timeout
...
// --- net.tcp_listen(host, port, backlog?) -> Listener ---
...
// --- Listener.accept(timeout_ms?) -> Connection? ---
...
// --- Listener.accept_raw(timeout_ms?) -> record{fd, addr, port} ---
...
// --- UdpSocket.send_to(data, host, port) -> i64 ---
...
// --- UdpSocket.recv_from(size?) -> record{data, addr, port}? ---
...
// --- UdpSocket.set_broadcast(enabled) -> nil ---
...
// --- net.resolve(hostname) -> list<string> ---
...
// --- net.parse_addr(address) -> record{host, port}? ---
AngaraObject Angara_net_parse_addr(int arg_count, AngaraObject* args) {
    ...
    // Strip brackets for IPv6 [::1]:port format
...
// Workaround: Listener.accept returns TcpConn, so we reference it
// We define the accept method class externally via the listener's class
