...
// path.stem(path) -> string  (filename without extension)
...
// path.normalize(path) -> string  (remove . and .., double slashes)
...
// path.parent(path) -> string  (like dirname but more intuitive)
AngaraObject Angara_path_parent(int arg_count, AngaraObject* args) {
    // Just alias to dirname logic
...
AngaraObject Angara_path_with_extension(int arg_count, AngaraObject* args) {
    if (arg_count != 2 || !IS_STR(args[0]) || !IS_STR(args[1])) {
        ang_api->throw_error("with_extension(path, ext) expects two strings."); return ang_nil();
    }
    const char* path = ang_api->as_cstr(args[0]);
