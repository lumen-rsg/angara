//
// process.c — Angara child process module
//
// Provides process spawning, execution, and pipe I/O.
...
// Execute a command via /bin/sh -c and capture stdout.
...
// --- process.run(cmd, args?, options?) -> record ---
// Run a command and capture exit_code, stdout, stderr.
// cmd: string, args: list<string>?, options: record?
// Options: { capture: bool (default true), cwd: string?, env: record? }
...
// --- process.spawn(cmd, args?) -> Process ---
// Spawn a process with stdin/stdout/stderr pipes for streaming I/O.
...
// --- Process.read_stdout(size?) -> string? ---
...
// --- Process.read_stderr(size?) -> string? ---
...
AngaraObject Angara_Process_is_alive(int arg_count, AngaraObject* args) {
    ...
    if (result == 0) return ang_bool(true); // still running
    pd->exited = 1;
    pd->exit_code = WIFEXITED(status) ? WEXITSTATUS(status) : -1;
