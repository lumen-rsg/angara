//
// random.c — Angara random module (xoshiro256** PRNG)
//
...
// --- xoshiro256** PRNG ---
// State is stored in a native instance so multiple generators can coexist.
...
// --- Global generator (seeded from time + address entropy) ---
...
// --- Global functions (use built-in global generator) ---

// random.int() -> i64  (random i64)
...
// random.int_range(min, max) -> i64  (inclusive range)
...
// random.seed(value) -> nil  (re-seed the global generator)
...
// random.bytes(n) -> string  (random byte string)
...
// random.pick(list) -> any  (random element from list)
...
// random.shuffle(list) -> nil  (in-place Fisher-Yates shuffle)
...
// random.uuid_v4() -> string  (proper UUID v4 using xoshiro)
