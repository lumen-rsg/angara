//
// rpc.c — Angara JSON-RPC 2.0 module
//
// Provides a TCP-based RPC mechanism using JSON-RPC 2.0 protocol.
// Uses newline-delimited JSON over TCP (NDJSON) for framing.
//
// Server usage (pull-based, no closure invocation needed):
//   let server = rpc.create_server(8080)
//   while true {
//     server.service()
//     let req = server.accept_request()
//     if req != nil {
//       // req has: id, method, params
//       let result = handle_it(req)
//       server.respond(req["id"], result)
//     }
//   }
//
// Client usage:
//   let result = rpc.call("localhost", 8080, "add", [1, 2])
//   rpc.notify("localhost", 8080, "log", ["hello"])
//
...
// ---------------------------------------------------------------------------
// Incoming request buffer (linked list)
// ---------------------------------------------------------------------------

typedef struct RpcRequest {
    int64_t         id;          ///< Request ID (-1 = notification, no reply expected)
    char*           method;      ///< Method name (copied)
    AngaraObject    params;      ///< Parsed params (Angara list or record)
    int             client_fd;   ///< Socket to respond on
    struct RpcRequest* next;
} RpcRequest;

// ---------------------------------------------------------------------------
// Server native instance
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// JSON ↔ Angara conversion (reuses json_bridge, same as json.c)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Read a full NDJSON message from a non-blocking socket.
// Returns a malloc'd C string (NULL if nothing ready / error / disconnect).
// The `out_closed` flag is set to 1 when the peer disconnected.
// ---------------------------------------------------------------------------

/// Read buffer per client_fd. We keep it simple: a small static buffer per call.
/// For a production server you'd want a connection struct with a persistent buffer.
/// We'll use a dynamically-growing approach.
...
static char* read_ndjson_message(int fd, int* out_closed) {
    *out_closed = 0;
    // We read byte-by-byte looking for '\n'. This is simple and correct for NDJSON.
    // For performance, you'd read in chunks, but this is fine for moderate loads.
    ReadBuffer buf;
    buf.cap = 4096;
    buf.len = 0;
...
// ---------------------------------------------------------------------------
// Send a full message over a socket (blocking send).
...
// ---------------------------------------------------------------------------
// Server: create_server(port) -> Server native instance
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Server.service() — poll for new connections and incoming data (non-blocking)
// ---------------------------------------------------------------------------
...
AngaraObject Angara_Server_service(int argc, AngaraObject args[]) {
    ...
    while (1) {
        ...
        if (client_fd < 0) break; // EWOULDBLOCK or error — no more pending
        set_nonblocking(client_fd);
        // We'll poll this fd next time service() is called
    }

    // Read from existing clients is deferred — accept_request does it on demand.
    // But we need to track client fds. For simplicity, we don't track them persistently
    // here; instead accept_request will try to read from recently-accepted fds.
    // 
    // A cleaner approach: on accept, store client_fd in a list, and service() reads
    // from all of them. But that requires more state. Let's keep it simple:
    // service() just accepts new connections. accept_request() will be called with
    // a client fd.
...
// ---------------------------------------------------------------------------
// Server.poll_clients() — returns a list of client file descriptors
//   that have data ready to read. Accepts pending connections and probes.
// ---------------------------------------------------------------------------
...
AngaraObject Angara_Server_poll_clients(int argc, AngaraObject args[]) {
    ...
    while (1) {
        ...
        set_nonblocking(client_fd);
        // Store in server's client list (reuse req_head linked list tail trick below)
        // Actually, we'll just add to a separate client fd list
        // For now, immediately add to poll result
...
// ---------------------------------------------------------------------------
// Server.read_request(client_fd) — read one JSON-RPC request from client
//   Returns a record: { id: i64, method: string, params: any, client_fd: i64 }
//   Returns nil if no complete request is available.
// ---------------------------------------------------------------------------
...
AngaraObject Angara_Server_read_request(int argc, AngaraObject args[]) {
    ...
    // args[0] is the Server, args[1] is client_fd
    // Actually, let's have args[0] = Server, args[1] = client_fd
    RpcServer* srv = (RpcServer*)ang_api->native_instance_data(args[0]);
    ...
    // Extract "jsonrpc" field — we're lenient, don't enforce "2.0" strictly
    // Extract "method" (required)
    // Extract "id" (optional — if absent, it's a notification)
        ...
        if (key && strcmp(key, "method") == 0) {
            ...
            if (id_val) {
                if (json_bridge_is_number(id_val)) {
                    ...
                    // String IDs — store as string, not i64
                    ...
                    // Don't set "id" field at all (caller checks nil)
                }
            }
    ...
    // Store client_fd so respond knows where to send
...
// ---------------------------------------------------------------------------
// Server.respond(id, result, client_fd) — send a JSON-RPC response
// ---------------------------------------------------------------------------

AngaraObject Angara_Server_respond(int argc, AngaraObject args[]) {
    // args[0] = Server, args[1] = id, args[2] = result, args[3] = client_fd
    if (argc < 4) {
    ...
    // Build JSON-RPC response: {"jsonrpc":"2.0","id":...,"result":...}
    ...
    if (ang_is_i64(args[1])) {
        ...
        // nil id — probably a notification, but send anyway
...
// ---------------------------------------------------------------------------
// Client: rpc.call(host, port, method, params) -> any
//   Makes a synchronous JSON-RPC call and returns the result.
// ---------------------------------------------------------------------------
...
AngaraObject Angara_rpc_call(int argc, AngaraObject args[]) {
    ...
    // Use a simple counter for request IDs (per-call, so always 1 for a single call)
    ...
    if (inet_pton(AF_INET, host, &addr.sin_addr) <= 0) {
        // Try localhost resolution for hostnames
        // Simple: just use 127.0.0.1 for "localhost"
    ...
    // Set a connect timeout (5 seconds)
    struct timeval tv;
    tv.tv_sec = 5;
    tv.tv_usec = 0;
    ...
    // Send request (NDJSON — append newline)
    ...
    // Simple approach: read until newline
    ReadBuffer rbuf;
    rbuf.cap = 8192;
    rbuf.len = 0;
...
// ---------------------------------------------------------------------------
// Client: rpc.notify(host, port, method, params) -> nil
