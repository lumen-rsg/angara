//
// term.c — Angara terminal utilities module
//
// Provides terminal control: colors, cursor movement, key reading,
// progress bars, and formatted tables. No external dependencies.
//
...
// --- term.set_color(fg, bg?) -> nil ---
// fg/bg are strings: "black","red","green","yellow","blue","magenta","cyan","white",
//                      "bright_black","bright_red",..., or "reset"
...
// --- term.terminal_size() -> record{rows, cols} ---
...
// --- term.read_key() -> string ---
// Reads a single keypress (handles escape sequences for arrow keys etc.)
AngaraObject Angara_term_read_key(int arg_count, AngaraObject* args) {
    ...
    if (buf[0] == 4)  return ang_api->string("eof");  // Ctrl+D
...
// --- term.progress_bar(current, total, width?) -> string ---
AngaraObject Angara_term_progress_bar(int arg_count, AngaraObject* args) {
    ...
    int filled = (int)(pct * (width - 3)); // Reserve space for " %NN"
    if (filled < 0) filled = 0;
    if (filled > width - 3) filled = width - 3;

    int bar_width = width - 5; // Reserve for " NNN%"
...
// --- term.colorize(text, fg, bg?) -> string ---
...
// --- term.table(headers, rows) -> string ---
