//
// unistd.c — Angara POSIX unistd module
//
// Low-level process, file descriptor, and POSIX file operations.
// Complements os, env, fs, and time modules.
//
...
// unistd.fork() -> i64  (child PID in parent, 0 in child, -1 on error)
...
// unistd.exec(path, args...) -> nil  (replaces current process; only returns on error)
AngaraObject Angara_unistd_exec(int arg_count, AngaraObject* args) {
    ...
    // Build argv: path + additional args + NULL
...
// unistd.waitpid(pid) -> record{pid: i64, exit_code: i64}
...
// unistd.pipe() -> record{read_fd: i64, write_fd: i64}
...
// unistd.write(fd, data) -> i64  (bytes written)
...
// unistd.lseek(fd, offset, whence) -> i64
...
// =============================================================================
// §3  File & Link Operations (POSIX-specific)
// =============================================================================

// unistd.unlink(path) -> nil
...
// unistd.symlink(target, linkpath) -> nil
...
// unistd.usleep(microseconds) -> nil
...
// unistd.alarm(seconds) -> i64  (returns previous alarm time)
